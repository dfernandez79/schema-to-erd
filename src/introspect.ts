...
const introspect = async (sql: SQL, schema: string, selected?: string[]): Promise<Schema> => {
  ...
  const included = resolveSelection(schema, available, selected);
  ...
  for (const row of columnRows) {
    ...
    const column: Column = {
      ...
      baseType: shortenSqlType(row.base_type),
...
const resolveSelection = (
  ...
  if (missing.length > 0) {
    throw new RunError(`table(s) not found in schema '${schema}': ${missing.join(", ")}`);
  }
  return new Set(selected);
};

const toKeySet = (rows: ColumnRefRow[]): Set<string> =>
